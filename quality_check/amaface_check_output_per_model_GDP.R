...
# we don't have leaf, wood, root respiration terms in GDAY
p4<-xyplot(I(RAU)~RAU,df0,groups=SIM,
           main='RAU~RAU',auto.key=T,
           scales=list(relation='free'),
           panel=function(...){
             panel.xyplot(...)
             panel.abline(a=0,b=1)})
...
# No fixation into plant
p26<-xyplot(I(NUP+NLRETR+NWRETR+NFRRETR+NCRRETR)~I(NGL+NGFR+NGCR+NGW),df0,groups=SIM,
...
# No fixation
p27<-xyplot(I(NUP+NLRETR+NWRETR+NFRRETR+NCRRETR-NGL-NGFR-NGCR-NGW)~deltaNSTOR,df0,groups=SIM,
            main='NUP+NLRETR+NWRETR+NFRRETR+NCRRETR-NGL-NGR-NGCR-NGW~deltaNSTOR',auto.key=T,
            scales=list(relation='free'),
            panel=function(...){
              panel.xyplot(...)
...
# add fixation 
p30<-xyplot(I(NDEP+NFIX+NLITIN+NWLIN+NCRLIN+NFRLIN-NUP-NLEACH-NVOL)~I(deltaNSOIL+deltaNFLIT+deltaNCLITB),df0,groups=SIM,
